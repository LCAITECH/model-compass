...
code is still what's stored on Context/AIModel; this only changes the
label shown to a person -- needed by decision/explainer/ (to phrase a
reason in English) and by interfaces/web/ (to label the dropdown), so
it lives here rather than under either one specifically.
"""
